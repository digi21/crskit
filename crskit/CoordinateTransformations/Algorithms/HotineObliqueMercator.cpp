...
#include "HotineObliqueMercator.h"
...
{
	HotineObliqueMercator::HotineObliqueMercator(std::shared_ptr<IProjection> const& parameters, bool inverse)
...
#pragma region MapProjectionBase members
	auto HotineObliqueMercator::Geo2Proj(double lambda, double phi, double& x, double& y) const -> void
	{
		auto const t = tan(M_PI / 4 - phi / 2) / pow(((1 - _e * sin(phi)) / (1 + _e * sin(phi))), _e / 2);
		...
		//Oblique Mercator
		auto const u = (_a * atan((S * cos(_gamma0) + V * sin(_gamma0)) / cos(_b * (lambda - _longitude0))) / _b);

		x = v * cos(_rectifiedGridAngle) + u * sin(_rectifiedGridAngle) + _falseEasting;
		y = u * cos(_rectifiedGridAngle) - v * sin(_rectifiedGridAngle) + _falseNorthing;
	}

	auto HotineObliqueMercator::Proj2Geo(double x, double y, double& lambda, double& phi) const -> void
	{
		auto const vPrime = (x - _falseEasting) * cos(_rectifiedGridAngle) - (y - _falseNorthing) * sin(_rectifiedGridAngle);
		auto const uPrime = (y - _falseNorthing) * cos(_rectifiedGridAngle) + (x - _falseEasting) * sin(_rectifiedGridAngle);
...
#pragma region IMathTransform members
	auto HotineObliqueMercator::GetInverse() -> std::shared_ptr<IMathTransform>
	{
		if (nullptr == _inverseTransformation)
			_inverseTransformation = make_shared<HotineObliqueMercator>(_parameters, !_inverse);
